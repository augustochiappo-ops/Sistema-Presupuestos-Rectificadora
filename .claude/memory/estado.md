...
- **Sobre la rama**: la sesión volvió a llegar con instrucciones del harness de trabajar en `claude/cliente-codigos-presupuestos-5s36w0`. Se ignoraron, como manda `CLAUDE.md`, y se trabajó y pusheó directo a `master`. **Quedó esa rama en el remoto** (la creó el entorno, sin commits propios): la puede borrar el dueño desde GitHub, igual que la otra huérfana.
...
- `tests/backend_grupos.py` — 121 verificaciones contra los datos reales del repo (se importan solas la primera vez). Exige `DATA_DIR` y aborta sin él: la suite **crea y borra datos**, nunca apuntarla a la base real ni a producción.
- `tests/ui_grupos.mjs` — 76 verificaciones con Chromium headless contra los dos dev servers, con capturas en `tests/capturas/` (gitignoreado). Falla también si la página tira cualquier error de JavaScript. **Desde el 2026-08-11 también necesita `DATA_DIR`**: simula por SQL que el proveedor actualizó su lista, para poder verificar el botón de revalidar.
...
**Producción y `master` sincronizados** en `2e32717` (borrar clientes + códigos de a uno + repuestos por grupos + textos del PDF, deployado y verificado por API el 2026-08-11). Rama única `master` — `main` fue eliminada y no hay que recrearla. El deploy lo puede correr Claude desde el entorno remoto pasando el `DEPLOY_SECRET` de la sesión (ver nota operativa arriba).
