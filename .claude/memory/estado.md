...
- **Ramas remotas obsoletas borradas** (todas ya contenidas en `master`, sin cambios propios): `claude/delete-budget-button-rxdu3j`, `claude/repuestos-tab-presupuestos-ykgkn8`, `claude/mobile-system-optimization-8cz2nw`, `claude/parts-search-categories-brands-bq6y9c`. La rama `claude/revisar-codigo-master-vye67l` (repuestos en presupuestos) también quedó confirmada como ya integrada a `master` sin cambios propios — falta solo el borrado remoto (bloqueado por el clasificador de permisos de Claude Code, pendiente de que el usuario lo corra a mano: `git push origin --delete claude/revisar-codigo-master-vye67l`).
