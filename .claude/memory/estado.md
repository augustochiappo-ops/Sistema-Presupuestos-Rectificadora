...
- `tests/backend_grupos.py` — 141 verificaciones contra los datos reales del repo (se importan solas la primera vez). Exige `DATA_DIR` y aborta sin él: la suite **crea y borra datos**, nunca apuntarla a la base real ni a producción.
- `tests/ui_grupos.mjs` — 120 verificaciones con Chromium headless contra los dos dev servers, con capturas en `tests/capturas/` (gitignoreado). Falla también si la página tira cualquier error de JavaScript. **Desde el 2026-08-11 también necesita `DATA_DIR`**: simula por SQL que el proveedor actualizó su lista, para poder verificar el botón de revalidar.
