...
## Próximo paso
Sistema en producción y funcionando, con deploy remoto automatizado (pero solo ejecutable por el usuario o un entorno con red habilitada hacia PythonAnywhere, ver nota arriba). Repositorio con una sola rama principal (`master`; `main` fue eliminada, ver arriba). **Producción quedó en el commit `f295774`** (ajuste % del wizard, PDF sin precios, edición de clientes, buscador difuso) — todavía faltan deployar cuatro bloques más de esta misma sesión larga: `75ded16` (ajuste % en el detalle, motores ya presupuestados primero), `9c72e84` (ajuste % persistente, detalle reordenado, repuestos usados por motor con ocultar, presupuestos vinculados), `df30ee3` (bloque unificado + botones junto a Volver, categoría en sugeridos, marca del sidebar, buscador de presupuestos) y `bfcc073` (filtro de cliente en el buscador de presupuestos, buscador en Clientes). Correr el mismo curl de deploy de siempre trae todo de una — no hace falta hacerlo en pasos. También queda: borrar el presupuesto de prueba #7 (tarea de background ya creada), definir el mecanismo de carga diaria del CSV de CRAC (único punto abierto de `INTEGRACION-PENDIENTE.md` — una vez resuelto, sacar `CRAC/precio-stock.csv` de git como ya se hizo con `Excel/Proveedor/`, para no inflar el historial con cada actualización), decidir si conviene unificar la grafía "Chiappo"/"Chicappo" entre el sidebar y el PDF, y considerar si vale la pena migrar a un plan pago de PythonAnywhere si el uso crece (más CPU/disco). Como puede haber más de una sesión de Claude tocando este repo en paralelo (celular + escritorio), conviene chequear ramas remotas pendientes al empezar cada sesión.
