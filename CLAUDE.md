...
- **Búsqueda por medidas**: sección propia del menú lateral que encuentra una pieza por sus medidas (Ø, largo, alto…) con tolerancia, cuando no se sabe el código. Los catálogos técnicos (camisas, válvulas, guías, asientos, subconjuntos, conjuntos, pistones, cojinetes de biela y bujes de biela) viven en `CRAC/tecnicos/*.json`; el precio y el stock salen del catálogo del proveedor ya importado. Cómo se leyó cada catálogo del fabricante está documentado al lado de los datos: `CRAC/tecnicos/CARGA-CONJUNTOS.md` y `CRAC/tecnicos/CARGA-COJINETES.md`.
